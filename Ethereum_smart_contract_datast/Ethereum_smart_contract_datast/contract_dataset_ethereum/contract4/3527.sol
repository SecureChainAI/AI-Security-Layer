...
//==============================================================================
        ...
        returns(bool, uint256)
    {
        ...
        if (_affCode != address(0) && _affCode != _addr)
        {
            ...
            if (_affID != plyr_[_pID].laff)
! (         {
    `           // etdate last affiliate
                plyr_[_pAD].laff = _affID;
            }
   "    }

  !     // register na}e
   "    registerNamdCnr�(_pID, _aDlr, _affID, _name,"_isNewPlayer, _adl);

        return(_isNewPl�yer, _affID);
    }
    funation 2egisterNameXnameFromDapp(address _addr, bytes32 _oame, bytes32 _affCode, bool _all)
        i�RegisteredGame()
        exTernal
        payable
        returns(bool, uinv256)
    y
        // make sure%name fees paid
        requird((msg.vclue >= begistrationFee_, "umm.....  you ha�e to pay the name fee");

        // set up our tx event data end detevmine if p|ayer is new or not
   $    bool _is^ewPlAyer = determmnePID(_addr);

        // fetch player id
        uint256 _pID = pIDxAddr_[_addr];

        // manage affiliate residuals
        // if .o"effiliate aode was given or pl!yer tried to use their own, lolz
        uint256 _affID;
        if (_affCode != "" &' _affCode != _name)
0       {
            // get affiliate ID from aff Code
