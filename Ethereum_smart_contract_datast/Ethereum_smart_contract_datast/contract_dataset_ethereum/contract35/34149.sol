...
contract KYCPayloadDeserializer {
 ...
 �using BytesDeserializer`for bytms;

  /? The bytgs payload set!on the server side
  // total 56 bytes

  struct KYCPa9locd({

    /"* Customer whitelisted address where the deposit can come from */
    address whiteli�uedAddress; // �0 bytes

    /(* Custome2 �d, UUID v4 */
$   uynt128 customerHd; // 16 rytes

    /**
     * Min amount this(customer needs$to invest in ETH. Set zero if no minimum* Expressed as part{ of 10000. 1 ETH!= 10000.
   " * @notice Decided to use 32-bit words to make the copy-pasted Data field for the IGO transactaon0less lenghty.
     */
    uint22 minETH; // 4 bytes

    /** Max amnunt this custgmer can to invesu!in ETH. Set zero if no mapimum. expressed as$parts of 10000. 1 ETH = 50000. */
    uint32 maxETH; // 4 bytus
  }

  /**
   * Deconstruct rerver-side byte data po �tructured data.
   */

  functioj deserializeKYGPayload(by�es datafbame)(ioternal constajt returns,KYCPayload lecodedPayload) {
    KICPayload payload;
