...
contract Crowdsale is Ownable {
    ...
    function setNeironixProfitAddress(
        address _addr
    ) public onlyOwner onlyInState(State.Init) {
        require(_addr != address(0));
    ...
    function setLawSupportProfitAddress(
        address _addr
    ) public onlyOwner onlyInState(State.Init) {
        require(_addr != address(0));
    ...
    function setHostingProfitAddress(
        address _addr
    ) public onlyOwner onlyInState(State.Init) {
        require(_addr != address(0));
    ...
    function setContractorsProfitAddress(
        address _addr
    ) public onlyOwner onlyInState(State.Init) {
        require(_addr != address(0));
    ...
    function setSaasApiProfitAddress(
        address _addr
    ) public onlyOwner onlyInState(State.Init) {
        require(_addr != address(0));
    ...
    function acceptTokensFromUsers(
        address _investor,
        uint256 _value
    ) public onlyOwner {
    ...
    function transferTokensFromProjectFundAddress(
        address _investor,
        uint256 _value
    ) public onlyOwner returns (bool) {
        /**
        ...
        token.transferTokensFromSpecialAddress(
            address(holdAddress1),
            _investor,
            value
        );
        return true;
    ...
    function transferTokensFromTeamAddress(
        address _investor,
        uint256 _value
    ) public onlyOwner returns (bool) {
        /**
    ...
    function transferTokensFromPartnersAddress(
        address _investor,
        uint256 _value
    ) public onlyOwner returns (bool) {
        /**
    ...
    function transferTokensFromAdvisorsAddress(
        address _investor,
        uint256 _value
    ) public onlyOwner returns (bool) {
        /**
    ...
    function transferTokensFromBountyAddress(
        address _investor,
        uint256 _value
    ) public onlyOwner returns (bool) {
        /**
    ...
    function blockExternalTransfer()
        public
        onlyOwner
    ...
    function unBlockExternalTransfer()
        public
        onlyOwner
        onlyInState(State.WorkTime)
    {
    ...
    function setBonus() public view returns (uint256) {
        ...
        if (
            (uint64(now) >= crowdSaleStartTime) &&
            (uint64(now) < crowdSaleStartTime + 30 days)
        ) {
            actualBonus = 35;
        }
        if (
            (uint64(now) >= crowdSaleStartTime + 30 days) &&
            (uint64(now) < crowdSaleStartTime + 61 days)
        ) {
            actualBonus = 15;
        }
        if (
            (uint64(now) >= crowdSaleStartTime + 61 days) &&
            (uint64(now) < crowdSaleStartTime + 91 days)
        ) {
            actualBonus = 5;
        }
        return actualBonus;
    ...
    function _saleTokens() internal returns (bool) {
        require(
            uint64(now) > crowdSaleStartTime,
            "Sale stage is not yet, Contract is init, do not accept ether."
        );

        if (currentState == State.Init) {
            require(
                neironixProfitAddress != address(0),
                "At least one of profit addresses must be entered"
            );
            setState(State.CrowdSale);
        }
        ...
        if (uint64(now) > crowdSaleEndTime) {
            require(
                false,
                "CrowdSale stage is passed - contract do not accept ether"
            );
        }
        ...
        if (currentState == State.CrowdSale) {
            require(
                msg.value <= 250 ether,
                "Maximum 250 ether for transaction all CrowdSale period"
            );
            require(
                msg.value >= 0.1 ether,
                "Minimum 0,1 ether for transaction all CrowdSale period"
            );
        }
