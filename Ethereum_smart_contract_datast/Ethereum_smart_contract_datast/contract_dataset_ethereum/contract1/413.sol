...
contract usingOraclize {
    ...
    bytes1 constant proofType_Android = 0x20;
    ...
    function oraclize_getPrice(
        string datasource
    ) internal oraclizeAPI returns (uint) {
    ...
    function oraclize_setProof(bytes1 proofP) internal oraclizeAPI {
    ...
    function getCodeSize(address _addr) internal constant returns (uint _size) {
        assembly {
    ...
    function stra2cbor(string[] arr) internal pure returns (bytes) {
        ...
        while (bytes1(cborlen).length > ctr) {
        ...
        for (i = 0; i < arrlen; i++) {
            ...
            for (uint x = 0; x < elemArray[i].length; x++) {
                ...
                if (x % 23 == 0) {
                    uint elemcborlen = elemArray[i].length - x >= 24
                        ? 23
                        : elemArray[i].length - x;
                    elemcborlen += 0x40;
                    uint lctr = ctr;
                    while (bytes1(elemcborlen).length > ctr - lctr) {
    ...
    function oraclize_newRandomDSQuery(
        ...
        oraclize_randomDS_setCommitment(
            queryId,
            keccak256(bytes8(_delay), args[1], sha256(args[0]), args[2])
        );
        return queryId;
    }
    ...
    function verifySig(
        ...
        sigs_ = copyBytes(
            dersig,
            offset + (uint(dersig[offset - 1]) - 0x20),
            32,
            sigs_,
            0
        );
    ...
    function oraclize_randomDS_proofVerify__main(
        ...
        if (!(keccak256(keyhash) == keccak256(sha256(context_name, queryId))))
            return false;

        bytes memory sig1 = new bytes(
            uint(proof[ledgerProofLength + (32 + 8 + 1 + 32) + 1]) + 2
        );
        copyBytes(
            proof,
        ...
        if (
            !matchBytes32Prefix(
                sha256(sig1),
                result,
                uint(proof[ledgerProofLength + 32 + 8])
            )
        ) return false;
        ...
        if (
            oraclize_randomDS_args[queryId] ==
            keccak256(commitmentSlice1, sessionPubkeyHash)
        ) {
...
contract CryptoCongress is StandardToken, SafeMath, usingOraclize {
    ...
    string public constant c =
        ").xpath(//*[contains(@class, 'TweetTextSize--jumbo')]/text())";
    ...
    uint256 public constant tokenCreationCap =
        131583 * (10 ** 3) * 10 ** decimals; // 131,583,000 tokens
    ...
    function createInitialAllotment(string username, uint256 initialAllotment) {
    ...
    function claimInitialAllotment(
        ...
        string memory url = usingOraclize.strConcat(
            a,
            username,
            b,
            twitterStatusID,
    ...
    function buyTokens(address beneficiary) public payable {
        ...
        uint256 checkedCrowdsaleSupply = safeAdd(
            totalSupplyFromCrowdsale,
            tokens
        );
    ...
    function propose(string _ID, string _description, string _data) {
        require(
            bytes(_ID).length < 281 &&
                bytes(_description).length < 281 &&
                bytes(_data).length < 281
        );
    ...
    function vote(string _proposalID, string _vote, string _data) {
        require(
            bytes(_proposalID).length < 281 &&
                bytes(_vote).length < 281 &&
                bytes(_data).length < 281
        );
