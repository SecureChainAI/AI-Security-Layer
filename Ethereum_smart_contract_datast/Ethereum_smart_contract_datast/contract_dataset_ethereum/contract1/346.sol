...
contract IOGToken is StandardToken, Ownable, Pausable {
    ...
    uint256 public constant TOTAL_SUPPLY =
        2200000000 * (10 ** uint256(decimals));
    ...
    function distribution(
        ...
        for (uint i = 0; i < addressList.length; i++) {
            ...
            if (0 < lockedPeriod) {
                timeLock(
                    addressList[i],
                    tokenAmountList[i] * (10 ** uint256(decimals)),
                    now + (lockedPeriod * 60 * 60 * 24)
                );
            }
            // unlock
            else {
    ...
    function canTransferIfLocked(address _sender) internal view returns (bool) {
        if (0 == addressLocks[_sender]) return true;
    ...
    function transfer(
        address _to,
        uint256 _value
    ) public canTransfer(msg.sender) whenNotPaused returns (bool success) {
    ...
    function transferFrom(
        ...
        address _to,
        uint256 _value
    ) public canTransfer(_from) whenNotPaused returns (bool success) {
